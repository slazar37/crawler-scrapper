...
public class ProductExtractor2
{
	...
	public static void main(String[] args) throws IOException, InterruptedException 
	{
		...
		// Local Variables:
		int fileCounter = 29; // Used to determine which file needs to be read.
		int max = 15; // Maximum time period for pausing execution.
		int min = 1; // Minimum time for pausing execution.
		
		String fileName = "results_redirect"; // Used to hold the path of the files to crawl.
		...
		org.jsoup.nodes.Document docProd_individual = null; // Establishes connection to individual product url.
		...
		fileName = "C:\\Users\\Stefan\\workspace\\Crawler\\links29.txt";
		try 
		{
			...
			while((line = read.readLine()) != null)
			{
		    	...
		    	docProd = Jsoup.connect(line).userAgent("Mozilla/5.0 (Windows; U; Windows NT 6.1; hu; rv:1.9.2.3) Gecko/20100401 Firefox/3.6.3 GTB7.1").timeout(20000).followRedirects(true).maxBodySize(1024*1024*3).referrer("http://www.yahoo.com").get(); // connect to the URL.
		    	// Pause for a random amount of time.
		    	//Thread.sleep((rand.nextInt((max - min) + 1) + min) * 1000); 
                
		    	// Check if it is not a redirect page:
		    	org.jsoup.nodes.Element original =  docProd.getElementById("olpDetailPageLink");
		    	try
		    	{
		    		 split_url = original.absUrl("href").split("\\?"); // Split the argument by the question mark
		    	}
		    	catch(NullPointerException nlp1)
		    	{
		    		System.out.println(nlp1.getMessage());
		    	}
		    	
		    	//System.out.println("url: " + split_url[0]);
		    	// Connect to individual product url:
		    	try
		    	{
			    	docProd_individual = Jsoup.connect(split_url[0]).userAgent("Mozilla/5.0 (Windows; U; Windows NT 6.1; hu; rv:1.9.2.3) Gecko/20100401 Firefox/3.6.3 GTB7.1").timeout(10000).followRedirects(true).maxBodySize(1024*1024*3).referrer("http://www.yahoo.com").get();
		    	}
		    	catch(NullPointerException nlp2)
		    	{
		    		System.out.println(nlp2.getMessage());
		    	}
				    ...
				    priceSell = docProd_individual.getElementsByClass("a-color-price");
				    ...
				    // Get the list price of the product:
				      priceList = docProd_individual.getElementsByClass("a-text-strike"); // Get the List Price.
				      if(priceList!=null && priceList.size() > 0)
				      {
					      try
					      {
					    	  ...
					    	  {
					    	        Elements tr = docProd_individual.select("span[class=a-size-medium a-color-price]");
					    	        if(!(tr.size() == 0))
					    	        {
					    	        	try
					    	        	{
				      ...
				      descDiv = docProd_individual.select("div[id=\"feature-bullets\"]");
				      ...
				      else
				      {
						...
						img = docProd_individual.select("img[id=landingImage]");
						String src[] = null;
						String src2[] = null;
				    	...
				    	tables = docProd_individual.select("table"); // Get all the tables from the file.
				    	int finder = 0;    
				    	...
				    	try
				    	{
				    		...
				    		scriptElements = docProd_individual.getElementsByTag("script");
